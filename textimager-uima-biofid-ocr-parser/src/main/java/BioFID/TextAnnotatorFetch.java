		...
		try {
			...
			if (remoteFiles.getBoolean("success")) {
				...
				try {
					...
					final AnalysisEngine conllEngine0 = AnalysisEngineFactory.createEngine(
							ConllBIO2003Writer.class,
							ConllBIO2003Writer.PARAM_TARGET_LOCATION, conllLocation + "si0",
							ConllBIO2003Writer.PARAM_CONLL_STRATEGY, 0,
							ConllBIO2003Writer.PARAM_OVERWRITE, true,
							ConllBIO2003Writer.PARAM_FILTER_FINGERPRINTED, true);
					
					final AnalysisEngine conllEngine1 = AnalysisEngineFactory.createEngine(
							ConllBIO2003Writer.class,
							ConllBIO2003Writer.PARAM_TARGET_LOCATION, conllLocation + "si1",
							ConllBIO2003Writer.PARAM_CONLL_STRATEGY, 1,
							ConllBIO2003Writer.PARAM_OVERWRITE, true,
							ConllBIO2003Writer.PARAM_FILTER_FINGERPRINTED, true);
					
					final AnalysisEngine conllEngine2 = AnalysisEngineFactory.createEngine(
							ConllBIO2003Writer.class,
							ConllBIO2003Writer.PARAM_TARGET_LOCATION, conllLocation + "si2",
					...
					remotePool.execute(() -> IntStream.range(0, rArray.length()).parallel().forEach(a -> {
						try {
							...
							if (documentJSON.getBoolean("success")) {
								...
								ImmutableList.of(conllEngine0, conllEngine1, conllEngine2).forEach(conllEngine -> {
									try {
