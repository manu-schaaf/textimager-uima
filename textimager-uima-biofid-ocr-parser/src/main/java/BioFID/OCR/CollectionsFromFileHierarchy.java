...
//			}
			...
			index = params.indexOf("--exportArticles");
			if (index > -1) {
				sArticleOutputPath = params.get(index + 1);
			...
			collectionDirs.parallelStream().forEach(documentParentDir -> {
				...
				files = getFilePaths(documentParentDir);
	...
	private static void printUsage() {
		System.out.println("Process XML Abby FineReader exports from entire collections by traversing a file hierarchy. The root path should be the top folder of a collection (Band/Zeitschrift).\n" +
				...
				"\t--exportArticles\tOptional, path to per article export location. If set all articles contained in the collections will be exported separately to the given path.");
	}
}
