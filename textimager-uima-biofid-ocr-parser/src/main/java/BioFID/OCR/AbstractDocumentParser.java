...
//						System.out.printf("\r%d/%d Wrote document %s.xmi", count, metadata.size(), documentId);
		...
		if (!sRawPath.isEmpty()) {
			...
			try (PrintWriter printWriter = new PrintWriter(new OutputStreamWriter(Files.newOutputStream(Paths.get(sOutputPath, documentId + "_orig.txt")), StandardCharsets.UTF_8))) {
