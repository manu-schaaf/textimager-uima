...
public class RunHeidelTimeDefaultLB {
    static final InputStream sentenceModelStream = RunHeidelTimeDefaultLB.class.getResourceAsStream("opennlp-de-ud-gsd-sentence-1.0-1.9.3.bin");
...
//        String pattern = "WP**/*.xml";
        ...
        try {
            ...
            LineBasedSentenceSegmenter lineBasedSentenceSegmenter = new LineBasedSentenceSegmenter();
            lineBasedSentenceSegmenter.initialize(UimaContextFactory.createUimaContext(
                    LineBasedSentenceSegmenter.PARAM_LANGUAGE, "de",
                    LineBasedSentenceSegmenter.PARAM_WRITE_SENTENCE, true
            ));
            ...
            while (reader.hasNext()) {
                try {
                    ...
                    int lastIndex = 0;
                    int nextIndex = documentText.indexOf("\n", lastIndex);
                    int counter = 0;
                    while (nextIndex > -1) {
                        createAnnotation(heidelTimeDefaultView, lastIndex, nextIndex, Sentence.class);
                        createAnnotation(heidelTimeDefaultView, lastIndex, nextIndex, de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Sentence.class);

                        lastIndex = nextIndex + 1;
                        nextIndex = documentText.indexOf("\n", lastIndex);
                    }
                    System.out.println();
                    System.out.flush();
